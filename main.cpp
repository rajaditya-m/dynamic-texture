...
void InitCylinder(double height,double lRad,double tRad,int nslices, int nstacks) 
{
  	...
  	point4* uniquePointsPool = new point4[numUniqueVertices];
  	vec3* uniqueNormalPool = new vec3[numUniqueVertices];
	...
	for (int j =0; j<nstacks; j++)
	{
		for (int i=0; i<nslices; i++) 
		{
	      	...
	      	uniquePointsPool[idx] = tempPoint;
	      	uniqueNormalPool[idx] = tempNormal;
	    }
	}
  	...
  	for (int j =0; j<nstacks-1; j++)
  	{
  		for (int i=0; i<=nslices; i++) 
  		{
      		...
      		points[n] = uniquePointsPool[idx];
      		normals[n++] = uniqueNormalPool[idx];
      		points[n] = uniquePointsPool[idx2];
      		normals[n++] = uniqueNormalPool[idx2];
    	}
  	}
}
