...
void store_ply(PlyFile* input, point ***vertices, Face ***faces,
  ...
  for(i = 0; i < input->num_elem_types; i = i + 1) {
    ...
    if(strcmp("vertex", element) == 0) {
      *vertices = (point**)malloc(sizeof(point) * count);
      *vertexcount = count;
	...
	if(strcmp("x", property->name) == 0 &&
	  ...
	  setup.internal_type = Float32;
	  setup.offset = offsetof(point, nx);
	  setup.count_internal = 0;
	  setup.count_offset = 0;
	  ...
	  *vertexnormals = 1;
	  ...
	  setup.internal_type = Float32;
	  setup.offset = offsetof(point, ny);
	  setup.count_internal = 0;
	  setup.count_offset = 0;
	  ...
	  *vertexnormals = 1;
	  ...
	  setup.internal_type = Float32;
	  setup.offset = offsetof(point, nz);
	  setup.count_internal = 0;
	  setup.count_offset = 0;
	...
	// dunno what it is
      ...
      //                (input, offsetof(point, struct_pointer));

      // copy the data
      for(j = 0; j < count; j = j + 1) {
	(*vertices)[j] = (point*)malloc(sizeof(point));
